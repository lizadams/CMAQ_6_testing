...
#When explicit providers fail, the most reliable solution for Intel MPI on a single node is to force it to use the tcp network provider, but constrain all traffic to the local loopback interface (127.0.0.1). This completely bypasses physical cluster networks while avoiding missing shared-memory drivers.
# Clear out the problematic provider flags
unsetenv FI_PROVIDER
unsetenv I_MPI_OFI_PROVIDER

# Force the stable TCP provider
setenv I_MPI_FABRICS ofi
setenv FI_PROVIDER tcp

# Force all TCP traffic to stay inside this single node (loopback)
setenv FI_TCP_IFACE lo

# Keep the Slurm integration flags that ensure stable startup
setenv I_MPI_PMI_LIBRARY libpmi2.so
setenv I_MPI_PIN 1
setenv I_MPI_PIN_RESPECT_CPU_SET 1
...
# ===================================================================
  ...
  ( srun --mpi=pmi2 -n $NPROCS $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt
