...
#SBATCH --mem=0    # --mem=0: Allocates all available memory on the node. If you share the node with others, replace 0 with a specific limit (e.g., --mem=64G). ## this didn't work
#SBATCH --mem=700G
## The Intel® Xeon® Gold 6140 Processor supports a maximum memory size of 768 GB per processor chip (dependent on memory type) and includes 24.75 MB of L3 cache memory.
#SBATCH -t 16:00:00
#SBATCH --export=ALL
#SBATCH --constraint=intel
##SBATCH --cpu-bind=cores  # this option wasn't recognized

## getting this error:
## srun: fatal: cpus-per-task set by two different environment variables SLURM_CPUS_PER_TASK=1 != SLURM_TRES_PER_TASK=cpu=4
...
## 1. Clear out conflicting Slurm structural limitations
#unsetenv SLURM_TRES_PER_TASK
#unsetenv SLURM_MEM_PER_CPU
#unsetenv SLURM_MEM_PER_NODE
#
## 2. Enforce the local Shared Memory fabric for Intel MPI (Crucial)
setenv I_MPI_FABRICS "shm"
setenv FI_PROVIDER "shm"
unsetenv FI_PROVIDER
unsetenv I_MPI_OFI_PROVIDER
#
## 3. Clean up task and affinity binding rules
#setenv SRUN_CPUS_PER_TASK 1
#setenv I_MPI_PIN off
...
#> Set General Parameters for Configuring the Simulation
 ...
 set APPL      = Bench_2022_12SE1  #> Application Name (e.g. Gridname)
...
#> Set Working, Input, and Output Directories
 setenv CMAQ_DATA /work/users/l/i/lizadams/CMAQv6.0_2022_12SE1_Benchmark_2day/
 setenv WORKDIR ${CMAQ_HOME}/CCTM/scripts          #> Working Directory. Where the runscript is.
 setenv OUTDIR  ${CMAQ_DATA}/output_CCTM_${RUNID}  #> Output Directory
 setenv INPDIR  ${CMAQ_DATA}/2022_12SE1            #> Input Directory
...
setenv GRID_NAME SE60BENCH           #> check GRIDDESC file for GRID_NAME options
...
#> Output Species and Layer Options
   ...
   setenv CONC_SPCS "O3 NO ANO3I ANO3J NO2 NH3 ANH4I ANH4J ASO4I ASO4J"
...
set ICpath    = $INPDIR/icbc/$MECH                        #> initial conditions input directory
set BCpath    = $INPDIR/icbc/$MECH                        #> boundary conditions input directory
set EMISpath  = $INPDIR/emis/$MECH/cmaq_ready             #> Emissions input directory
set IN_LTpath = $INPDIR/lightning                         #> lightning NOx input directory
set METpath   = $INPDIR/met/mcipv5.3.3                    #> meteorology input directory
...
# =====================================================================
  ...
  if ($NEW_START == true || $NEW_START == TRUE ) then
     setenv ICFILE ICON_v60_intel23.2_2022hd_12SE1_20220630.nc
  ...
  #> Boundary conditions, clean for hemispheric
  set BCFILE = BCON_v60_intel23.2_2022hd_12SE1_202207.nc 
  ...
  setenv CMAQ_MASKS $SZpath/OCEAN_${MM}_L3m_MC_CHL_chlor_a_12SE1.nc

  #> Gridded Emissions files
  setenv N_EMIS_GR 2
  set EMISfile  = emis_mole_all_${YYYYMMDD}_12SE1_nobeis_norwc_${MECH}_2022hd.ncf 
  ...
  set EMISfile  = emis_mole_rwc_${YYYYMMDD}_12SE1_${MECH}_2022hd.ncf 
  ...
  set STKCASEE = ${MECH}_2022hd           # In-line Emission Rate File Suffix
  set STKCASEG = ${MECH}_2022hd           # Stack parameter File Suffix

  setenv STK_GRPS_001 ${EMISpath}/ptnonipm/stack_groups_ptnonipm_${STKCASEG}.ncf
  setenv STK_GRPS_002 ${EMISpath}/ptegu/stack_groups_ptegu_${STKCASEG}.ncf
  setenv STK_GRPS_003 ${EMISpath}/canmex_point/stack_groups_canmex_point_${STKCASEG}.ncf
  setenv STK_GRPS_004 ${EMISpath}/ptagfire/stack_groups_ptagfire_${YYYYMMDD}_${STKCASEG}.ncf
  setenv STK_GRPS_005 ${EMISpath}/ptfire-wild/stack_groups_ptfire-wild_${YYYYMMDD}_${STKCASEG}.ncf
  setenv STK_GRPS_006 ${EMISpath}/ptfire-rx/stack_groups_ptfire-rx_${YYYYMMDD}_${STKCASEG}.ncf
  setenv STK_GRPS_007 ${EMISpath}/ptfire_othna/stack_groups_ptfire_othna_${YYYYMMDD}_${STKCASEG}.ncf
  setenv STK_GRPS_008 ${EMISpath}/pt_oilgas/stack_groups_pt_oilgas_${STKCASEG}.ncf
  setenv STK_GRPS_009 ${EMISpath}/cmv_c1c2_12/stack_groups_cmv_c1c2_12_${STKCASEG}.ncf
  setenv STK_GRPS_010 ${EMISpath}/cmv_c3_12/stack_groups_cmv_c3_12_${STKCASEG}.ncf
  setenv STK_GRPS_011 ${EMISpath}/ptnonipm_hr/stack_groups_ptnonipm_hr_${STKCASEG}.ncf

  setenv STK_EMIS_001 ${EMISpath}/ptnonipm/inln_mole_ptnonipm_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_002 ${EMISpath}/ptegu/inln_mole_ptegu_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_003 ${EMISpath}/canmex_point/inln_mole_canmex_point_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_004 ${EMISpath}/ptagfire/inln_mole_ptagfire_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_005 ${EMISpath}/ptfire-wild/inln_mole_ptfire-wild_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_006 ${EMISpath}/ptfire-rx/inln_mole_ptfire-rx_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_007 ${EMISpath}/ptfire_othna/inln_mole_ptfire_othna_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_008 ${EMISpath}/pt_oilgas/inln_mole_pt_oilgas_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_009 ${EMISpath}/cmv_c1c2_12/inln_mole_cmv_c1c2_12_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_010 ${EMISpath}/cmv_c3_12/inln_mole_cmv_c3_12_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_011 ${EMISpath}/ptnonipm_hr/inln_mole_ptnonipm_hr_${YYYYMMDD}_${STKCASEE}.ncf
  ...
  #> In-line lightning NOx options
     ...
     if ( $USE_LTNG_DATA == Y ) then
        setenv LTNG_DATA ${IN_LTpath}/WWLLNs_12km_60min_${YYYYMMDD}.ioapi.nc
	...
	# LNO_OPTION 5:  use synergized GLM/WWLLNs Energy with ICCG adjustment to set upper bound
     ...
     setenv LTNGPARMS_FILE ${IN_LTpath}/LTNG_AllParms_12SE1.nc #> lightning parameter file; ignore if LTNGPARAM = N
  endif

  if( $USE_SEGA_N == 'Y' ) then
     setenv SEGA_SOILINIT   $OUTDIR/CCTM_SSOILOUT_${RUNID}_${YESTERDAY}.nc
     if( $USE_SEGA_N_EF == 'Y') then
        setenv SEGA_EF ${INPDIR}/surface/BEIS4_SEGA_beld6_norm_emis_12SE1.ncf
     endif
  endif
  ...
  if ( $CTM_BIOGEMIS_MG == 'Y' ) then
    setenv MEGAN_SOILINP    $OUTDIR/CCTM_MSOILOUT_${RUNID}_${YESTERDAY}.nc
         ...
         setenv MEGAN_CTS $INPDIR/surface/megan3.2/CT3_12SE1.ncf
         setenv MEGAN_EFS $INPDIR/surface/megan3.2/EFMAPS_12SE1.ncf
         setenv MEGAN_LDF $INPDIR/surface/megan3.2/LDF_12SE1.ncf
         if ($BDSNP_MEGAN == 'Y') then
            setenv BDSNPINP    $OUTDIR/CCTM_BDSNPOUT_${RUNID}_${YESTERDAY}.nc
            setenv BDSNP_FFILE $INPDIR/surface/megan3.2/FERT_12SE1.ncf
            setenv BDSNP_NFILE $INPDIR/surface/megan3.2/NDEP_12SE1.ncf
            setenv BDSNP_LFILE $INPDIR/surface/megan3.2/LANDTYPE_12SE1.ncf
            setenv BDSNP_AFILE $INPDIR/surface/megan3.2/ARID_12SE1.ncf
            setenv BDSNP_NAFILE $INPDIR/surface/megan3.2/NONARID_12SE1.ncf
         endif
  endif
  if ( $CTM_BIOGEMIS_BE == 'Y' ) then   
     set IN_BEISpath = ${INPDIR}/surface 
     setenv GSPRO      		${BLD}/gspro_biogenics.txt
     setenv BEIS_NORM_EMIS      $IN_BEISpath/beis4_beld6_norm_emis.12SE1.ncf 
  ...
  setenv OCEAN_1 $SZpath/OCEAN_${MM}_L3m_MC_CHL_chlor_a_12SE1.nc

  #> Bidirectional ammonia configuration
  if ( $CTM_ABFLUX == 'Y' ) then
     # need to modify for FEST-C v1.4.
     setenv E2C_SOIL ${EPICpath}/${YYYY}r1_EPIC0509_12SE1_soil.nc
     setenv E2C_CHEM ${EPICpath}/${YYYY}r1_EPIC0509_12SE1_time${YYYYMMDD}.nc
     setenv E2C_LU ${EPICpath}/beld4_12SE1_2011.nc
  endif
...
#        setenv PACM_INFILE ${NMLpath}/pa_${MECH}.ctl
        setenv PACM_INFILE $WORKDIR//pa_cb6r5_ae7_aq_pox.ctl
...
# ===================================================================
  ...
  #( srun  -n 18 -c 1 --cpu-bind=none $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt
  # ( srun --cpus-per-task=1 -n 16 $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt
  # ( srun -genv I_MPI_PIN=1 -genv I_MPI_PIN_DOMAIN=core -n 16 ) |& tee buff_${EXECUTION_ID}.txt   # this works, but runs on only 1 processor
   ( srun --cpu-bind=none -n 16  $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt
