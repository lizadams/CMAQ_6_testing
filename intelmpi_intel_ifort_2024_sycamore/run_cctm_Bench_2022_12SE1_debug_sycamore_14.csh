...
#The error Invalid MPI type 'pmi' means that your cluster's Slurm installation does not support or compile the legacy --mpi=pmi or --mpi=pmi2 plugins. Instead, your cluster is strictly built to use modern PMIx.
#Because your system mandates PMIx, we must return to a PMIx launch configuration, but we will bypass your node's broken Libfabric hardware detection (fi_getinfo) by switching the backend engine to the native Shared Memory (shm) protocol built directly into Intel MPI.
#The Fix

# Force Intel MPI to use native shared memory bypasses 
setenv I_MPI_FABRICS shm
setenv I_MPI_SHM 1

# Completely clear the Libfabric engine flags that caused the fi_getinfo crash
unsetenv FI_PROVIDER
unsetenv I_MPI_OFI_PROVIDER
unsetenv I_MPI_OFI_LIBRARY_INTERNAL

# Re-enable the PMIx bootstrap layer required by your Slurm installation
...
# ===================================================================
  ...
  ( srun --mpi=pmix $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt
